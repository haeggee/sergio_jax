...
class SergioJAX:
    ...
    def custom_graph(
        ...
        assert contribution_rates.shape == (
            self.n_cell_types,
            self.n_genes,
            self.n_genes,
    ...
    def hill_function(
        self, x: jax.Array, h: jax.Array, n: Union[jax.Array, float] = 2.0
    ) -> jax.Array:
    ...
    def init_gene_concentration(
        ...
        def update(_, gene_exp):
            production_rate = self.production_rate(
                gene_exp, graph, basal_rates, contribution_rates, hill
            )
    ...
    def _simulate(
        ...
        final_exp, gene_exps = jax.lax.scan(
            step, init_exp, jnp.array(rngs), length=steps
        )
        # [n_steps, n_cell_types, n_genes]
        return gene_exps

    def convert_to_UMIcounts(self, rng: jax.random.PRNGKeyArray, scData: jax.Array):
    ...
    def simulate(self, rng: jax.random.PRNGKeyArray, kout: Optional[jax.Array] = None):
    ...
    def outlier_effect(
        ...
        rng, sample_rng = jax.random.split(rng)
        out_indicator = jax.random.bernoulli(
            sample_rng, p=outlier_prob, shape=(n_genes,)
        )

        #### generate outlier factors ####
        rng, sample_rng = jax.random.split(rng)
        outFactors = jax.random.normal(sample_rng, shape=(n_genes,)) * scale + mean
    ...
    def lib_size_effect(
        ...
        rng, sample_rng = jax.random.split(rng)
        libFactors = (
            jax.random.normal(sample_rng, shape=(n_cell_types, number_sc)) * scale
            + mean
        )
        # lognormal is the exp of normal samples
        libFactors = jnp.exp(libFactors)

        def lib_size_per_bin(binExprMatrix, binFactors):
            normalizFactors = jnp.sum(binExprMatrix, axis=0)
            binFactors = binFactors / jnp.where(
                normalizFactors == 0.0, 1.0, normalizFactors
            )
            binFactors = binFactors.reshape(1, number_sc)
            binFactors = jnp.repeat(binFactors, n_genes, axis=0)
            return binExprMatrix * binFactors

        ret_data = jax.vmap(lib_size_per_bin, (0, 0))(scData, libFactors)
        return libFactors, ret_data

    @staticmethod
    @jit
    def dropout_indicator(
        rng: jax.random.PRNGKeyArray,
        scData: jax.Array,
        shape: int = 1,
        percentile: int = 65,
