...
class Gene(object):
    def __init__(self, geneID, geneType, binID=-1):
        ...
        self.k = (
            []
        )  # For dynamics simulation it stores k1 to k4 for Rung-Kutta method, list of size 4 * num_c_to_evolve
        self.k_S = (
            []
        )  # For dynamics simulation it stores k1 to k4 for Rung-Kutta method, list of size 4 * num_c_to_evolve
...
class Sergio:
    def __init__(
        ...
        safety_steps=0,
        # dynamics and differentiation parameters
        dynamics=False,
        ...
        self.level2verts_ = {}
        self.gID_to_level_and_idx = (
            {}
        )  # This dictionary gives the level and idx in self.level2verts_ of a given gene ID
        self.binDict = (
            {}
        )  # This maps bin ID to list of gene objects in that bin; only used for dynamics simulations
        self.maxLevels_ = 0
    ...
    def custom_graph(self, *, g, k, b, hill):
        ...
        assert np.allclose(
            g[np.diag_indices(g.shape[0])], 0.0
    ...
    def init_gene_bin_conc_(self, level):
        ...
        for g in currGenes:
            ...
            if self.kout[gID]:
                assert not self.kdown[
                    gID
                ], "Cannot knockout and knockdown the same gene"
                interv_factor = 0.0
            elif self.kdown[gID]:
                interv_factor = 0.5
            else:
            ...
            if g[0].Type == "MR":
                ...
                for bIdx, rate in enumerate(allBinRates):
                    g[bIdx].append_Conc(
                        np.true_divide(interv_factor * rate, self.decayVector_[g[0].ID])
                    )
    ...
    def simulate(self, kout=None, kdown=None):
        if kout is None:
            kout = np.zeros(self.nGenes_).astype(bool)
        self.kout = kout

        if kdown is None:
            kdown = np.zeros(self.nGenes_).astype(bool)
        self.kdown = kdown
    ...
    def lib_size_effect(self, scData, mean, scale):
        ...
        for binExprMatrix, binFactors in zip(scData, libFactors):
            ...
            binFactors = binFactors / np.where(
